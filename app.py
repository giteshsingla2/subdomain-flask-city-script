...
class DatabaseCache:
    ...
    def _load_data(self):
        with sqlite3.connect('/var/www/yourapp/locations.db') as conn:
            ...
            for row in cursor.fetchall():
                self.states[row['state_abbr']] = row['state_name']
                
            # Load cities with their states
            cursor.execute("SELECT city_name, state_abbr FROM Cities")
            for row in cursor.fetchall():
                if row['state_abbr'] not in self.cities:
                    self.cities[row['state_abbr']] = []
                self.cities[row['state_abbr']].append(row['city_name'])
                
            # Load zip codes
            cursor.execute("SELECT city_name, zip_code FROM CityZipCodes")
            for row in cursor.fetchall():
                city_name = row['city_name'].lower()
                if city_name not in self.zip_codes:
                    self.zip_codes[city_name] = []
                self.zip_codes[city_name].append(row['zip_code'])
                
                # Load neighborhoods
                cursor.execute("SELECT city_name, neighborhood FROM Neighborhoods")
                for row in cursor.fetchall():
                    city_name = row['city_name'].lower()
                    if city_name not in self.neighborhoods:
                        self.neighborhoods[city_name] = []
                    self.neighborhoods[city_name].append(row['neighborhood'])
    ...
    def replace_placeholders(text, service_name, city_name, state_abbreviation, state_full_name, required_data, zip_codes=[], neighborhoods=[], city_zip_code=""):
        ...
        text = text.replace("[Service]", service_name)\
                ...
                .replace("[Neighborhoods]", ", ".join([n.get('name', '') for n in neighborhoods if n and isinstance(n, dict)]))\
    ...
    def generate_map_embed(city, state):
    ...
    def get_neighborhoods_from_db(city_name):
        return db_cache.neighborhoods.get(city_name.lower(), [])
            ...
            if len(subdomains) == 1:
                ...
                if city_info:
                    ...
                    for neighborhood in neighborhoods:
                        formatted_neighborhood = '+'.join(neighborhood.split()) if neighborhood else ''
                    ...
                    map_embed_url = generate_map_embed(city_name, state_abbreviation)
